...
import { XAXIS1, YAXIS1, YAXIS2, ZAXIS1 } from './Constants.jsx';
import * as IMM from '../DS4/Animations/Immersive.jsx';

extend({ Text });

const clips = IMM.getClips();
...
function Line({ start, end, color, dashed=false, ...props }) {
  ...
  useFrame(() =>{
    ...
    ref.current.scale.set(Math.abs(end[0] - start[0]), Math.abs(end[1] - start[1]), Math.abs(end[2] - start[2]));
  })
  ...
  const LineInstance = useMemo(() =>
    <line ref={ref}>
      ...
      <If if={props.dashed == true}>
        <lineDashedMaterial transparent dashSize={1} gapSize={0.5} color={color} />
      </If>
      <If if={props.dashed != true}>
        <lineBasicMaterial transparent color={color} />
      </If>
    </line>
  ,[])
...
function ChangePoint({ pos, text, start, end, color, linewidth, ...props }) {
  return(
    <>
      <DreiLine
        points={[start, end]}
        ...
        dashed={true}
      />
      <TextBox color={color} anchorX={"center"} lookAt={false} position={[(start[0] + end[0]) * pos, (start[1] + end[1]) * 0.5 + 2, (start[2] + end[2]) * 0.5]} text={text} />
    </>
  )
}
...
function TextBox({position=[0, 0, 0], lookAt=true, text, textType="default", anchorX="left", anchorY="middle", label, ...props}){
  ...
  function getOpts(type){
    return type == "title"? {
      font: "Noto Sans",
      fontSize: props.fontSize? props.fontSize:7.5,
      ...
      font: "Noto Sans",
      fontSize: props.fontSize? props.fontSize:2.5,
      color: props.color?props.color:"black",
      maxWidth: 65,
      lineHeight: 1.15,
      letterSpacing: 0,
  ...
  const Text = useMemo(() =>
    <group ref={refPosition} position={position}>
      <text {...opts} ref={ref} text={text + ""} font={fonts[opts.font]} anchorX={anchorX} anchorY={anchorY}>
        <meshBasicMaterial attach="material" fog={false} color={opts.color} side={THREE.DoubleSide} />
      </text>
    </group>
  , []);
  ...
  const Rect = useMemo(() =>
    <group ref={main}>
      <mesh raycast={() => null}>
        ...
        <meshStandardMaterial fog={false} ref={mat} color={color} transparent={true} />
      </mesh>
      <mesh raycast={() => null}>
...
const DS3Camera = React.forwardRef((props, ref) => {
  useLayoutEffect(() => {
    // console.log(ref);
  }, [])
...
export {Line, ChangePoint, TextBox, Rect, TextComponent, statesConverter, AnimationGenerator, OrthoCamera, DS3Camera, MiniMap, If, Lerp};
