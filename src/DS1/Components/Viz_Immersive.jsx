  ...
  <>
    {
      Array(ySteps).fill(0).map((x, y) => x + y).map((item, idx) => {
        return <mesh key={idx} position={[yPadding + item * ((yLength - 2 * yPadding) / (ySteps - 1)), -tickLength, -tickLength]}>
          <If if={step >= 6}>
            <Line key={idx} color={"black"} start={[0, tickLength, tickLength]} end={[0, tickLength, 0]} /> // Tick
          ...
          <If if={step >= 6}>
              ...
              <Line key={idx+1000} color={"lightgrey"} start={[0, tickLength, tickLength]} end={[0, -8*rectDepth, tickLength]} />
          </If>
        </mesh>
      })
    }
