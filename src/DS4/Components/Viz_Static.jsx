      ...
      <>
        {
          [...Array(21).keys()].map(i => i * 10 + 1820).map((year)=>{
            ...
            return(
              <If if={xPos >= -0.01 && xPos <= len_year+0.01}>
                <group
                  key={'year_' + year + '_text_line'}
                  position={[xPos, 0, 0]}>
                  <If if={step!=2}>
                    <Text position={[0, 0, gap_na]} text={String(year)} label={'xLC'} />
                    <DreiLine points={[[0, 0, 0], [0, 0, len_tick]]} lineWidth={0.4} color={'#444444'} />
                  </If>
                  <If if={step==2}>
                    <Text position={[0, -gap_na, 0]} text={String(year)} label={'-xyLC'} />
                    <DreiLine points={[[0, 0, 0], [0, -len_tick, 0]]} lineWidth={0.4} color={'#444444'} />
                  </If>
                </group>
              </If>
            )
        ...
        <DreiLine points={[[-gap_ca, 0, 0], [len_year, 0, 0]]} lineWidth={0.4} color={'#444444'} />
      </>
    )
  }, [step==2, ranges]);
      ...
      <>
        {
          [...Array(9).keys()].map(i => i * 10 + 0).map((val) =>{
            ...
            return(
              <If if={zPos >= gap_ca - 0.01 && zPos <= len_age + 0.01}>
                <group key={'age_' + val + '_text_line'} position={[0, 0, zPos]}>
                  <Text position={[-gap_na, 0, 0]} text={String(val)} label={'xCT'} />
                  <DreiLine points={[[0, 0, 0], [-len_tick, 0, 0]]} lineWidth={0.4} color={'#444444'} />
                </group>
              </If>
            )
          })
        ...
        <DreiLine points={[[0, 0, gap_ca], [0, 0, len_age + gap_ca]]} lineWidth={0.4} color={'#444444'} />
      </>
    )
  }, [step==2, ranges]);
            ...
            return(
              <group key={'mor_' + val + '_text_line'}>
                <Text
                  ...
                  label={'yRC'} />
                <DreiLine
                  points={[[0, height, 0], [-len_tick, height, 0]]}
                  lineWidth={0.4} color={'#444444'} />
              </group>
            )
          })
        }
        <Text position={[0 - 1, len / 2, 0]} text={'Mortality Rate(%)'} label={'-x1CB'} />
        <DreiLine points={[[0, 0, 0], [0, len, 0]]} lineWidth={0.4} color={'#444444'} />
      </>
    )
  }, []);
...
function HeatmapAtAge0(){
  ...
  return (
    <instancedMesh ref={ref} args={[null, null, total_data.length]}>
      <boxGeometry />
      <meshPhongMaterial transparent={true} clippingPlanes={cuttingPlanes} />
    </instancedMesh>
  )
}
...
function ImgPlane4({idx = 0, ...props}){
  ...
  const ImgPlane = useMemo(() => {
    const xScale = (2019 - 1816) / (2000 - 1900);
    const zScale = (80 - 0) / (70 - 10);

    return(
      <group
        position={[
          (imgPos[idx][0] - 1900) * size * xScale,
          20,
          ((70 - imgPos[idx][1]) * size) * zScale - 2.7
        ]}
...
function Highlight5({startX, startY, rad, xLen, yLen, idx, ...props}){
  ...
  useFrame(() => {
    ref.current.traverse(child => {
      if(child.material){
        child.material.opacity = (step==5 && idx-10 == step5Clip)? 1:0;
      }
    });
  })
...
function ImgPlane5({idx = 0, ...props}){
  ...
  const onClick = useCallback((e) => {
    if(step5Clip == idx){
      setRanges('whole', [0, 80, 1816, 2019]);
      setStep5Clip(idx);
    }else{
      setRanges('whole', presetRanges[idx]);
      setStep5Clip(idx);
    }
  }, [])
...
function Heatmap(){
  ...
  useFrame(()=>{
    if(step == 4){
      ...
      setRanges('whole', [
        prevRanges['age'][0]  + diffRanges['age'][0],
        prevRanges['age'][1]  + diffRanges['age'][1],
        prevRanges['year'][0] + diffRanges['year'][0],
        prevRanges['year'][1] + diffRanges['year'][1],
      ]);
    }
  })
...
const VisComponent = React.forwardRef((props, ref) =>{
  ...
  const visibleArr = {
    ...
    HeatmapAtAge0   : [true,  true,  true,  false, true,  true],
  ...
  return(
    <group position={[0, 0, 0]} ref={ref}>
      ...
      <group visible={visibleArr.LineAtAge[step]}>
        <LineAtAge data={data} age_start={18} age_end={40} />
      </group>
      <group visible={visibleArr.Axis_Years[step]}>
        <Axis_Years />
      </group>
      <group visible={visibleArr.Axis_Ages[step]}>
        <Axis_Ages />
      </group>
